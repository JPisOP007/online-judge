...
- **JavaScript Harness:** Submissions run inside a harness that pre-reads stdin (exposing `readline()` / `readAll()`) and shadows `require`, `process`, `module` and `globalThis`, since `require('child_process')` would otherwise be a one-line escape. Because lexical shadowing is escapable from global scope — the Function constructor is reachable as `(()=>{}).constructor` without ever naming `Function` — Node is additionally launched with `--disallow-code-generation-from-strings`, which refuses `eval`, `Function()` and the constructor variants at the engine level.
